    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-[#f0f0f0] font-display text-slate-900 overflow-hidden relative">
      ...
      <div className="w-full max-w-[480px] space-y-8 relative z-10">
        ...
        <main className="bg-white border-3 border-black shadow-neo rounded p-8 md:p-12 space-y-8 transition-all duration-200">
          ...
          {/* Error Message */}
          {error && (
            <div className="bg-red-100 border-3 border-black text-red-800 px-4 py-3 font-bold text-sm shadow-neo-sm">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleLogin} className="flex flex-col gap-6">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 px-6 flex items-center justify-center gap-2 group bg-[#fbef23] border-3 border-black shadow-neo-sm rounded hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-neo-hover active:translate-x-[4px] active:translate-y-[4px] active:shadow-none transition-all duration-100 mt-2 disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {loading ? (
                <Loader2 className="w-6 h-6 text-black animate-spin" />
              ) : (
                <>
                  <span className="block text-xl font-black text-black leading-none uppercase">
                    Login
                  </span>
                  <ArrowRight
        ...
        <footer className="flex flex-wrap justify-center gap-x-8 gap-y-2 py-4">
          <a
            href="#"
            className="flex items-center gap-1 text-sm font-bold underline decoration-3 decoration-[#137fec] underline-offset-4 hover:text-[#137fec] transition-colors"
          >
            <HelpCircle size={14} /> Help Desk
          </a>
          <a
            href="#"
            className="flex items-center gap-1 text-sm font-bold underline decoration-3 decoration-black underline-offset-4 hover:text-[#137fec] transition-colors"
          >
