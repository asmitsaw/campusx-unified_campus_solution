...
export default function Dashboard() {
    ...
    const isEligible = true; // can be computed from CGPA/backlogs when profile API is wired
                ...
                <div className="bg-neo-blue border-4 border-black p-6 shadow-[8px_8px_0px_0px_#000] flex flex-col md:flex-row items-center justify-between gap-6 transform rotate-0 hover:rotate-1 transition-transform">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            {latestApp ? (
                                <>
                                    <h4 className="text-black font-black text-xl uppercase bg-white px-2 border-2 border-black inline-block mb-1">
                                        {latestApp.placements?.company} — {latestApp.status}
                                    </h4>
                                    <p className="text-sm font-bold text-black">
                                        {placementStats?.active_drives || 0} active drive(s) · {placementStats?.students_placed || 0} placed so far
                                    </p>
                                </>
                            ) : (
                                <>
                                    ...
                                    <p className="text-sm font-bold text-black">
                                        {placementStats?.active_drives || 0} drives open · Start applying now!
                                    </p>
                                </>
                            )}
                        </div>
                    </div>
                    <button onClick={() => navigate('/dashboard/placement')} className="px-6 py-3 bg-black text-white text-sm font-black uppercase border-2 border-white shadow-[4px_4px_0px_0px_#fff] hover:bg-white hover:text-black hover:border-black hover:shadow-[4px_4px_0px_0px_#000] transition-all">
                        View Companies
                    </button>
                </div>
            </div>
