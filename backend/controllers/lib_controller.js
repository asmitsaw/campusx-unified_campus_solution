...
export const getMessages = async (req, res) => {
  try {
    ...
    let query = supabase.from("lib_messages").select("*").order("created_at", { ascending: true });
    
    if (student_id) {
      // If student_id provided, get messages where they are sender or receiver
      query = query.or(`sender_id.eq.${student_id},receiver_id.eq.${student_id},sender_id.eq.librarian,receiver_id.eq.librarian`);
      // Filter for the specific student's thread
      query = query.or(`sender_id.eq.${student_id},receiver_id.eq.${student_id}`);
    }
...
// Get all students who have chatted (for librarian view)
export const getChatUsers = async (req, res) => {
  try {
    // Get unique sender_ids where role is student
    const { data, error } = await supabase
      .from("lib_messages")
    ...
    const studentIds = [...new Set(data.map(m => m.sender_id))];
